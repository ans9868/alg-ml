...
\begin{itemize}
	...
	\item $w$: width of each Array[row] 
...
Let the population size be $n$. Split into $C$ groups of equal size $n/C$.

\textbf{Scheme.}

Test each group once (level 1).  
If a group tests $(+)$, retest every member of that group individually (level 2).

\textbf{Worst-case counting.}
...
Suppose there are $k$ infected individuals total.  
In the worst case, each infected person is in a different group, so at most $\min(k,C)$ groups are positive. Hence
\[
\#\{\text{positive groups}\} \le k.
\]
Each positive group triggers $n/C$ individual retests, so
\[
t_2 \le k\cdot \frac{n}{C}.
\]
Therefore the total number of tests satisfies
\[
T = t_1+t_2 \le C + k\frac{n}{C}.
\]

Let $a := kn$. Consider the nonnegative square
\[
\left(\sqrt{C}-\frac{\sqrt{a}}{\sqrt{C}}\right)^2 \ge 0.
\]
Expanding gives
\[
C - 2\sqrt{a} + \frac{a}{C} \ge 0
\quad\Longrightarrow\quad
C + \frac{a}{C} \ge 2\sqrt{a}.
\]
Substituting $a=kn$,
\[
C + k\frac{n}{C} \ge 2\sqrt{nk}.
\]

So the best choice is to take the balance point
\[
C = \sqrt{\frac{n}{k}},
\]
which makes the two terms equal, and yields
\[
T \le C + k\frac{n}{C}
= \sqrt{\frac{n}{k}} + k\frac{n}{\sqrt{n/k}}
= 2\sqrt{nk}.
\]

\[
\boxed{T \le 2\sqrt{nk}.}
\]
* Vishal collaborated with me on this problem.
...
Genuinly tried these problems on paper. I had a friend explain but I didn't totally understand so I didn't want to write that in. I may ask for help in office hours for this. "I don't know".
\end{enumerate}
...
No Collaborators except for problem 3.1. (I wish I had more collaborators, I need to do this).
